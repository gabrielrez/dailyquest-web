            ...
            <div class="fixed inset-0 bg-[#020202]/50 backdrop-blur-xs flex items-center justify-center z-50">
                <div
                    ...
                    <div class="flex items-center justify-between">
                        <h3 class="text-white text-xl font-semibold">Create Goal</h3>
                    ...
                    <form class="space-y-5" @submit.prevent="createGoal">
                        <div>
                        ...
                        <div>
                            <label class="block text-sm font-medium text-white mb-2.5">Status</label>
                            <select id="status" name="status" v-model="status" required
                                class="w-full rounded-[6px] bg-[#09090B] border border-[#27272A] text-[#A1A1AA] px-4 py-2">
    ...
    async function createGoal() {
        ...
        try {
            const {data} = await api.post(`/collections/${props.collection.id}/goals`, {
                name: name.value,
                description: description.value,
                status: status.value,
            });

            emit("goalCreated", data.data);
            emit("close");
        } catch (error) {
